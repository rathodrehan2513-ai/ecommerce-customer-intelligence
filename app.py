...
# ----------------- REAL GOOGLE AUTHENTICATION & CREDENTIALS -----------------
USER_CREDENTIALS = {
    ...
    with col2:
        ...
        with st.container(border=True):
            ...
            if google_auth_configured:
                ...
                st.info("Add `[google_auth]` credentials to your Streamlit secrets to enable Google Login.")

            st.markdown('<div class="auth-separator">OR CONTINUE WITH PASSWORD</div>', unsafe_allow_html=True)

            # Manual Password Form
            with st.form("login_form"):
                username = st.text_input("Username", placeholder="e.g. RehanRathod2513")
...
# Determine if user is logged in via Google SSO or Manual Password
...
if not (is_google_logged_in or is_manual_logged_in):
    ...
    # Sidebar Navigation & Account Management
    with st.sidebar:
        st.markdown(f"👤 **Logged in as:** `{display_user}`")
        if is_google_logged_in and "email" in st.session_state.get("user_info", {}):
